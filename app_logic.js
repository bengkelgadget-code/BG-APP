...
function calculateDynamicMargin(layananName, nominal) {
    ...
    for (var i = 0; i < marginData.length; i++) {
        ...
        let isNewFormat = (row[1] === 'Range Nominal' || row[1] === 'Persentase');
        ...
        if (layanans.includes(layananName.toUpperCase())) {
            ...
            if (tipe === 'Range Nominal') {
                var minNom = parseInt(param1Str.replace(/[^0-9]/g, '')) || 0;
                var maxNomStr = param2Str.replace(/[^0-9]/g, '');
                var maxNom = maxNomStr ? parseInt(maxNomStr) : Infinity; 
                
                if (nominal >= minNom && nominal < maxNom) {
                    return parseInt(marginStr.replace(/[^0-9]/g, '')) || 0;
                }
            } 
...
async function editDataGen(displayIndex) {
    try {
        ...
        if (currentConfig.sheet === 'Pengaturan_Margin') {
             ...
             setTimeout(() => {
                 let tipe = isNewFormat ? rowData[1] : 'Range Nominal';
                 let elTipe = document.getElementById('tipe_margin');
                 if(elTipe) {
                     elTipe.value = tipe;
                     if(window.jQuery) $(elTipe).trigger('change.select2');
                 }
                 window.toggleMarginFields(); 

                 document.getElementById('id_margin').value = rowData[0];
                 
                 let layananStr = isNewFormat ? String(rowData[2]) : String(rowData[1]);
                 if(window.jQuery) $('#layanan_margin').val(layananStr.split(', ')).trigger('change');

                 let p1 = isNewFormat ? String(rowData[3]) : String(rowData[2]);
                 let p2 = isNewFormat ? String(rowData[4]) : String(rowData[3]);
                 let marg = isNewFormat ? String(rowData[5]) : String(rowData[4]);
...
function initGenericForm() {
    ...
    if (currentConfig && currentConfig.sheet === 'Pengaturan_Margin') {
        setTimeout(() => { window.toggleMarginFields(); }, 50);
    }
}
...
async function handleFormSubmit(e, formEl) {
    ...
    if (currentConfig.sheet === 'Pengaturan_Margin') {
        ...
        if (!layanan) {
            Swal.fire('Perhatian', 'Layanan Terkait wajib dipilih!', 'warning');
            window.isSubmittingMaster = false;
            return;
        }
